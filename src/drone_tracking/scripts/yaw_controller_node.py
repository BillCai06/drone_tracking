...
class YawControllerNode:
    def __init__(self):
        ...
        self.pub_cmd    = rospy.Publisher("/px4ctrl/ext_yaw_rate",
        ...
        rospy.Subscriber("/detector_node/detection", Detection, self._det_cb,  queue_size=5)
    ...
    def _control_loop(self, _event):
        ...
        if not do_control:
            rospy.logwarn_throttle(3.0,
                f"[yaw_ctrl] IDLE — enabled={enabled}  offboard={is_offboard}(mode='{mode}')  "
                f"det_fresh={det_fresh}(detected={det.detected if det else 'None'} "
                f"age={det_age:.2f}s)")
